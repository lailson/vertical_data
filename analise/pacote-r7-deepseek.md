# Rodada 7 — REVISÃO DO M0. Eixo técnico.

A outra sessão executou o M0: conector CADURB + validador de completude offline, a partir da spec
pública. Também produziu a metodologia do índice (Anexo 2). Eu verifiquei e confirmei: obrigatoriedade
do schema exata, 15 de 15 códigos de variáveis corretos, denominador corrigido (Anexo 1, seções 1-2).

Três pontos para você julgar:

1. **k-means substituiu o índice com pesos** (Anexo 2, seção 3). Argumento deles: "sem peso
   inventado". Meu contra-argumento (Anexo 1, seção 5): k-means não elimina arbitrariedade, apenas a
   esconde (k=3, escolha das 4 variáveis, min-max sensível a outlier), e é MENOS defensável perante
   TCE que coeficientes declarados. Além disso é instável: incluir um município novo pode remapear
   grupos de todos os bairros. **Quem está certo? Há forma de ter os dois?**
2. **`V05000` = "domicílios em setor ESCOLHIDO para aplicação do entorno"** (Anexo 1, seção 3) —
   o entorno é aplicado a setores selecionados. Em Teresina a cobertura é 99,9%, mas em outro
   município pode não ser. Como tratar isso corretamente no pipeline e no painel?
3. **O M0 tem lacunas que eu apontei** (Anexo 1, seção 7): spec é `0.0.1-SNAPSHOT` sem teste de
   divergência; o fluxo de geometria (`idLotePonto`/`idLotePoligono`, envio por lote + vinculação)
   pode não estar coberto; validação semântica vs sintática. **Priorize: o que é bloqueante para
   vender o diagnóstico e o que pode esperar?**

Responda no máximo 6 achados. Adversarial, concreto. Cite "Anexo N".
Foque no que muda a decisão técnica, não em elogio ao trabalho feito.
