...
# RODADA 8 — M0 destravado; e-SIC nº 2 morto; laudo com cara de produto (16/09/2026)

## Decisões aceitas integralmente

1. **e-SIC SEFAZ-PI morto por design** — pedido de *elaboração de estudo* não é acesso a documento; resposta previsível "pedido indevido" queimando 15+10 dias. Registrado em `docs/esic-textos.md` (v2). A linha de R$/município que ele buscaria já existe pública: **ITBI por município no SICONFI**.
2. **Insight ITBI > IPTU no interior adotado como linha de deck** (Cocal 3,3× · Floriano 1,8× · Bom Jesus 1,7×): o tributo imobiliário que o interior arrecada é o de **transação**, e transação exige **registro** — onde o CIB trava. Receita corrente, hoje, verificável; melhor que hipótese de IBS futuro.
3. **Três correções aceitas**: laudo tem custo real de ingestão (1–2 dias/município — núcleo da precificação); contrato separa **obrigação (laudo)** de **marco condicionado (remessa — depende do Termo do município, fora do nosso controle)**; preço via **cashback** (R$ 3–5 mil com reembolso integral na assinatura em 60 dias, 10 primeiros, contrapartida de calibração).
4. **Enquadramento**: o laudo mede distância a uma norma com <1 ano (LC 214), não a competência do secretário. Réus na reunião: prazo federal e cartório. Pilotos REURB + os 8 do RREO não têm dono de cadastro a ofender.
5. Lista anti-adiamento e o teste de 7 dias (≥2 agendas marcadas ou o bloqueio é execução) — anotados como critério de próxima revisão.

## Verificações desta rodada

**Reprodução parcial da tabela SICONFI (ITBI×IPTU).** A API pública tem os dados por município (MSC orçamentária, classe 6, naturezas 1121.01xx = IPTU e 1121.02xx = ITBI, somando 12 meses). Meus totais para Teresina 2025: **IPTU R$ 71,7 mi / ITBI R$ 44,9 mi** — mesma estrutura, valores **menores** que os da outra sessão (R$ 166,3/60,8 mi), que devem vir do export do portal SICONFI (classificação RREO) ou de outra matriz. **Ação antes de ir ao deck: fixar UM método canônico e conferir contra o TCE-PI** (não apresentar números de fontes misturadas).

**Fluxo de credencial (correção nº 2).** Substância confirmada: a credencial depende de ato do município (Termo de Adesão assinado por representante legal) — fora do controle do fornecedor. O detalhe e-CAC/ICP-Brasil/DOU não está no manual público (fica no Roteiro Técnico, acesso restrito aos conveniados); assumir como restritivo por segurança contratual.

## Artefatos produzidos

- **`m0-conector/LAUDO-completude.html`** — laudo com cara de produto (identidade do painel: KPIs coloridos por aptidão, falhas por regra com barras, cobertura estratégica, enquadramento correto no rodapé). Gerado pelo `validador_completude.py` a partir de qualquer CSV municipal.
- `docs/esic-textos.md` v2 — item 2 marcado MORTO POR DESIGN com a justificativa e a alternativa.

## Pendências

Método canônico ITBI/IPTU (acima) · credencial CADURB (e-mail) · TCE-PI Altos/Paulistana · revalidação RREO · dossiê de dispensa dos 8 · 1 página de credencial técnica · **teste das 7 dias: ≥2 agendas**.

---
---

# RODADA 9 — Item A fechado: método fiscal canônico definido (16/09/2026)

Veredito em `docs/metodo-fiscal-itbi-iptu.md`: **o número do deck é o do portal SICONFI** (classificação RREO 1.1.1.2.1.01/02 — a que os municípios reportam e o TCE confere). A soma direta da API MSC é um **campo minado**, demonstrado com três métodos e valores distintos para o mesmo município-ano (portal 166/60,8 mi · MSCC 71,7/44,9 mi · MSCE 53,3/31,2 mi para Teresina) e com **ITBI = R$ 0** em Floriano, Corrente, Cocal e Altos sob códigos 112102xx (lançamento via outros códigos) — evidência em `dados/fiscais-itbi-iptu-mscc.json`. Um deck com número de API crua seria desmontado por qualquer secretário de fazenda.

**O insight central sobrevive**: ITBI>IPTU no interior confirma direcionalmente sob método independente (Bom Jesus 2,08× vs 1,73×; Teresina 0,63 vs 0,37 — interior transaciona, capital patrimonia). Regra: portal para citar; API só após capturar o endpoint real do portal (browser) e validar 3 municípios; nunca misturar fontes na mesma tabela.

**Bônus da rodada:** API do TCE-PI quebrada por extração do bundle JS da SPA (`/prefeituras` com id+codIBGE das 224; `/receitas/{id}/{exercicio}`) — o endpoint de receitas só devolve top-10 transferências, mas o de prefeituras é útil para o E2 (unidades gestoras por município).

---
---

# RODADA 10 — Itens B e C entregues: dossiê de dispensa + credencial técnica (16/09/2026)

**B — `dossie-dispensa/` (5 arquivos):** processo passo a passo com calendário de
empenho (decisão até 20/11 · contrato até 30/11 · empenho até 10/12); fichas dos
8 municípios do sinal RREO com dados de contexto (IBGE, TCE id, bairros 2–14,
população, tenant Foxinline — União e Ilha Grande com incumbente → abordagem
complementar); TR modelo com a separação **obrigação (laudo, 15 dias úteis) ×
marco condicionado (remessa, depende do Termo de Adesão do município)**; minuta
de decisão de dispensa (art. 75, II + PNCP art. 176) e declaração de necessidade.
**Alerta comercial registrado:** cashback é arriscado com ente público — para
municípios, desconto formal justificado na estimativa; rebate só no privado.

**C — `docs/credencial-tecnica.md`:** 1 página para entrar em qualquer reunião —
o que está construído e demonstrável (painel real, conector da spec pública,
laudo, metodologia, dossiê), a tabela de ofertas, postura LGPD/sem-prazo-alheio,
e o porquê-agora (31/12/2026 · PI só Teresina transmitiu).

**Fila técnica esvaziada.** Restam pendências de dependência externa: credencial
CADURB (e-mail), validação manual dos 6 números do portal SICONFI (30 min),
revalidação do método RREO, TCE-PI de Altos/Paulistana via browser, INEP (último).
O teste das 7 dias (≥2 agendas até 23/09) é agora o único medidor que importa.

---
---

# RODADA 11 — Rodada 9 da outra sessão absorvida; §§ verificados no TCU; TR v2 (16/09/2026)

## Verificação legal (fonte: TCU — licitacoesecontratos.tcu.gov.br)

- **Art. 75, §2º = duplicação dos limites para consórcio público** (e autarquia/fundação
  agências executivas). **A outra sessão estava certa — e o meu dossiê tinha os §§
  invertidos** (citava §2º como vedação ao fracionamento). **Corrigido**: a vedação ao
  fracionamento decorre do **§1º** (aferição por somatório do exercício da unidade
  gestora + objetos de mesma natureza). Fonte: [TCU — Dispensa em razão do valor](https://licitacoesecontratos.tcu.gov.br/5-10-2-1-dispensa-em-razao-do-valor-incisos-i-e-ii-2/).
- Teto consorciado de serviços: **R$ 130.984,22** (2 × 65.492,11) — distinto do teto
  simples de engenharia **R$ 130.984,20** (diferença de 2 centavos por coincidência;
  em documento para jurídico, confundir é caro). Montar consórcio não cabe na janela
  2026 — o TR nasce modular (escopo por aderente, preço unitário por imóvel).

## Decisões absorvidas nos artefatos desta máquina

1. **Dispensa única vence** (o calendário decide, não o jurídico): dois ciclos de
   processo em 20/11→10/12 não cabem; encadeadas empurram a conformidade para a LOA
   2027. → **TR reescrito (v2)**: pacote único R$ 15–25 mil (Fase 1 laudo obrigação
   com prazo + Fase 2 remessa marco com critério objetivo), manutenção opcional
   (entrada + 1º ano ≤ R$ 33 mil), TR alternativo de laudo isolado como fallback.
2. **Cashback morto** (o alerta partiu deste dossiê e pegou erro do parecer da rodada
   7 da outra sessão). Substitutos registrados no 00: empenho único como filtro,
   **mini-diagnóstico de 72h na pré-venda**, desconto formal na estimativa. "10 vagas"
   = declaração de capacidade (2 contas/mês), não promoção.
3. **Credencial técnica v2**: "menos de 4% dos 5.570 municípios até set/2026"
   (robusto a +34 transmissões), oferta com conformidade primeiro, "por que agora"
   com a linha ITBI/registro, item conector reformulado com honestidade.
4. **Fichas reordenadas por ITBI**: Bom Jesus nº 1 (R$ 61/hab, 2º per capita), depois
   Floriano e Corrente; Cocal vira narrativa, não alvo. Os 8 do RREO seguem como
   qualificação secundária.

## Engenharia do laudo (o item "3–5 dias" iniciado)

- **Spec pinada**: `m0-conector/spec/PIN.md` (sha256 `8b6c4aa5…`, versão, data) —
  regra: se a spec mudar, recapturar e regenerar regras, nunca editar à mão.
- **Primeira tabela de domínio extraída e integrada**: tipoLogradouro oficial
  (175 códigos, manual v1.12) em `spec/dominios.json` + regra semântica no validador
  (testada: código 999 rejeitado; 100/Avenida aceitos). Faltam: CEP × município e
  área × tipologia.

---
---

# RODADA 12 — Revisão jurídica do TR v2 absorvida integralmente; TR v2.1 (16/09/2026)

**B1 confirmado e corrigido:** a corrupção era parcialmente real no arquivo
("desetup", um "версões" cirílico; o mojibake matemático era o renderizador da
outra sessão). TR reescrito em texto limpo e **varredura de caracteres suspeitos
em todos os .md do projeto: nenhum restante**.

**Aplicado na TR v2.1** (`dossie-dispensa/02-termo-referencia-modelo.md`):
- **B2**: gatilho dos 70% desvinculado da emissão de CIB (ato da RFB) → agora
  "relatório final de conformidade OU relatório de impedimento aceitos"; CIB
  explicitado como resultado esperado, nunca condição.
- **B3**: mini-diagnóstico de 72h removido do TR (ação comercial; termo de
  confidencialidade próprio se houver acesso a dado pré-contrato).
- **B5**: seção 6 nova — **PI retida com licença de uso ao município** e **uso de
  dados agregados/anonimizados para calibração** (vedada identificação/transferência,
  município controlador) — as duas cláusulas constitutivas do ativo.
- **M1–M6**: imóvel analisado = registro único por inscrição (duplicatas contam 1×
  e viram falha); aceite do Laudo com rito (5 dias úteis, silêncio = aceite tácito
  para pagamento); vigência por prazo determinado com arts. 105–107; sanções
  sucintas (art. 156); Fase de gestão declarada fora do objeto principal; devolução
  de arquivos em 30 dias com declaração de expurgo.

**B4 absorvido no dossiê:** art. 75, **§3º** (verificado no TCU/lei): aviso de
intenção por ≥3 dias úteis em sítio oficial — calendário recalculado (**aviso até
12/11**), passo 3.5 no processo, **regulamento municipal do §3º promovido a item
de qualificação de lead** nas fichas, e a leitura estratégica registrada: o aviso
expõe o objeto e acorda a incumbente — resposta = proposta de referência pronta +
diferencial técnico explícito no TR (metodologia publicada, pin de versão,
vocabulário oficial de falhas).

**Mantido intacto (o "não se mexe" da revisão):** estrutura de fases com naturezas
distintas; critério de aceite da Fase 2 como obrigação de meio; preço setup +
unitário; a nota do §1º no valor; o TR alternativo com "jamais oferta principal".
