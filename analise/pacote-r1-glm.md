# Parecer solicitado — Rodada 1, eixo NEGÓCIO, RISCO E ESTRATÉGIA DE ENTRADA

Você é consultor sênior de estratégia em govtech brasileira. Uma empresa de software avalia
ENTRAR neste projeto. Leia o dossiê e responda no eixo NEGÓCIO/RISCO — NÃO no eixo técnico.

Foco das suas críticas (nesta ordem de prioridade):
1. A dependência de um fornecedor terceiro (Foxinline/CERURB) que é também concorrente
   adjacente e cuja infraestrutura pública está fora do ar. Qual o risco real e como neutralizá-lo.
2. A janela regulatória CIB/SINTER (obrigatório para todos os municípios em jan/2027) — isso
   muda o produto, o comprador, o preço e a urgência? Como posicionar?
3. Propriedade do dado: que cláusulas contratuais tornam este negócio defensável, e quais
   armadilhas de LGPD/licitação (Lei 14.133) existem ao vender para prefeitura.
4. Modelo de receita: projeto único vs. SaaS recorrente vs. licenciamento por município.
   Como escapar de "consultoria que termina" e virar produto replicável.
5. O menor escopo que fecha o primeiro contrato, prova valor e não cria dívida impagável.
6. Sinais de alerta que deveriam fazer a empresa NÃO entrar neste projeto.

Regras: seja adversarial, não elogie. Quantifique quando puder. Cite "seção N" do dossiê.
Máximo 6 achados, cada um com diagnóstico em uma frase e consequência em uma frase.
Sem resumo final, sem repetir o que o dossiê já diz.
