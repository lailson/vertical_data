# Rodada 2 — CONVERGÊNCIA. Eixo técnico/arquitetura.

Você deu o parecer técnico da Rodada 1 (Anexo 3). Depois disso, o reconhecimento revelou fatos
que **derrubam parcialmente o seu Achado 1 e o Achado 1 do parecer de negócio** (Anexo 2, itens A e B):
existem microserviços `api-exportacao` e `api-cerurbjus-integracao` vivos, e a Foxinline tem ~236
tenants municipais em 5 estados.

Responda APENAS o que muda de decisão, em no máximo 6 achados:

1. Com `api-exportacao` existindo, qual é a ordem de preferência REAL das vias de obtenção do dado
   CERURB, e qual você recomenda tentar primeiro? Scraping ainda tem algum lugar?
2. O parecer de negócio (Anexo 4, Achado 5) propõe como v1 um "diagnóstico de conformidade CIB"
   sobre dado aberto (IBGE + SICONFI + cadastro da Fazenda), sem CERURB e sem cartografia.
   **Isso é tecnicamente construível em 60–90 dias? O que exatamente entra e o que não entra?**
   Seja concreto: componentes, pipeline, modelo de dados.
3. Dado o Anexo 2 item F (APIs testadas e funcionando, SIDRA instável), qual a arquitetura de
   ingestão mínima e resiliente? Onde NÃO usar Base dos Dados/BigQuery e por quê.
4. Como se calcula honestamente "potencial de IPTU não arrecadado" com dado público + cadastro da
   Fazenda? Qual a metodologia defensável e qual a margem de erro que deve ser declarada?
5. O módulo de coleta própria: qual é o MENOR recorte dele que já gera ativo de dado defensável,
   sem virar "outro produto" como você alertou?
6. Onde você ainda discorda do parecer de negócio (Anexo 4), tecnicamente.

Seja adversarial. Cite "Anexo N, item/achado X".
